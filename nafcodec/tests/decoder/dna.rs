...
fn decode() {
    ...
    assert_eq!(str::from_utf8(&seq).unwrap().chars().filter(|&x| x == 'A').count(), 62115);
    assert_eq!(str::from_utf8(&seq).unwrap().chars().filter(|&x| x == 'C').count(), 28747);
    assert_eq!(str::from_utf8(&seq).unwrap().chars().filter(|&x| x == 'G').count(), 30763);
    assert_eq!(str::from_utf8(&seq).unwrap().chars().filter(|&x| x == 'T').count(), 61152);
...
fn mask() {
    ...
    let seq = r1.sequence.unwrap();
    assert!(str::from_utf8(&seq[..657]).unwrap().chars().all(|x| x.is_uppercase()));
    assert!(str::from_utf8(&seq[657..676]).unwrap().chars().all(|x| x.is_lowercase()));
    assert!(str::from_utf8(&seq[676..1311]).unwrap().chars().all(|x| x.is_uppercase()));
    assert!(str::from_utf8(&seq[1311..1350]).unwrap().chars().all(|x| x.is_lowercase()));

    let r2 = decoder.next().unwrap().unwrap();
    assert_eq!(r2.id.unwrap(), "test2");
    let seq = r2.sequence.unwrap();
    assert!(str::from_utf8(&seq[..525]   ).unwrap().chars().all(|x| x.is_uppercase()));
    assert!(str::from_utf8(&seq[525..621]).unwrap().chars().all(|x| x.is_lowercase()));
    assert!(str::from_utf8(&seq[621..720]).unwrap().chars().all(|x| x.is_uppercase()));
    assert!(str::from_utf8(&seq[720..733]).unwrap().chars().all(|x| x.is_lowercase()));
...
fn force_nomask() {
    ...
    assert_eq!(r1.id.unwrap(), "test1");
    let seq = r1.sequence.unwrap();
    assert!(str::from_utf8(&seq[..]).unwrap().chars().all(|x| x.is_uppercase()));
